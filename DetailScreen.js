...
export default class DetailScreen extends Component {
        ...
        fetch(url)
            ...
            .then((data) => {
                ...
                for (let i = 0; i < data.length; i++) {
                    let dataArray = [];
                    dataArray.push(data[i].sets)
                    dataArray.push(data[i].reps)
                    dataArray.push(data[i].weight)
                    sets.push(dataArray)
                }
    ...
    saveSets() {
        ...
        fetch('http://localhost:8080/jaxrs_war_exploded/v1/se/updateDetailTable?workout='+this.state.workout+'&exercise='+this.state.exercise+'&details='+dbValue)
    }
